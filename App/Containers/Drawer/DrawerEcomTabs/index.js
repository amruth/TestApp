...
import { Text,Image, StatusBar, Platform, ImageBackground,Dimensions,TouchableOpacity, ListView,View,TextInput} from 'react-native';
...
export default class DrawerEcomTabs extends Component {
  ...
  renderSelectedTab () {
    switch (this.state.selectedTab) {
      case 'Home':
        return (<ContainTabNearBy/>);
        break;
      case 'Category':
        return (<ContainTabCategory/>);
        break;
      case 'Discovery':
        return (<ContainTabDiscovery/>);
        break;
      case 'Gift':
        return (<ContainTabGift/>);
        break;
      case 'Profile':
        return (<ContainTabProfile/>);
        break;
      default:
    }
  }
      ...
      <Container style={styles.main}>
        <View style={styles.header}>
          <View style={styles.searcgBarBg}>
            ...
            <Ionicons name="ios-search" size={22} color='#a0a0a0' style={styles.searchIcon}/>
            <TextInput
          ...
          <TouchableOpacity style={styles.cartImgBg}>
            <Image style={styles.cartImage} source={Images.cart_icon}></Image>
          </TouchableOpacity>
        </View>
      <Content>
      ...
      <Footer>
        <FooterTab style={styles.footerTabBg}>
          <Button vertical
            ...
            <Text style={(this.state.selectedTab == 'Home') ? [styles.activeTabText,{marginTop: Metrics.WIDTH * 0.01}] : [styles.normalTabText,{marginTop: Metrics.WIDTH * 0.01}]}>Nearby</Text>
          </Button>
          <Button vertical
            ...
            <Text style={(this.state.selectedTab == 'Category') ? [styles.activeTabText,{marginTop: Metrics.WIDTH * 0.01}] : [styles.normalTabText,{marginTop: Metrics.WIDTH * 0.01}]}>Category</Text>
          </Button>
          <Button vertical
            onPress={() => this.setState({selectedTab: 'Discovery'})}>
            {
              (this.state.selectedTab == 'Discovery') ?
              <Ionicons name="ios-albums-outline" size={24} color="#00bff3" style={{justifyContent: 'center'}}/>
              :
              <Ionicons name="ios-albums-outline" size={24} color='#929292'/>
            }
            <Text style={(this.state.selectedTab == 'Discovery') ? styles.activeTabText : styles.normalTabText}>Discovery</Text>
          </Button>
          <Button vertical
            ...
            <Text style={(this.state.selectedTab == 'Gift') ? [styles.activeTabText,{marginTop: Metrics.WIDTH * 0.01}] : [styles.normalTabText,{marginTop: Metrics.WIDTH * 0.01}]}>Gift</Text>
          </Button>
          <Button vertical
            ...
            <Text style={(this.state.selectedTab == 'Profile') ? styles.activeTabText : styles.normalTabText}>Profile</Text>
          </Button>
        </FooterTab>
      </Footer>
      </Container>
